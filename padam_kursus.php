<?php
	...
	if (isset($_GET['ID']))
	{
		...
		// delete the entry
		$result = mysql_query("DELETE FROM COURSE WHERE CR_ID = '$ID'")
		or die(mysql_error()); 
		...
		if ($result)
		{
		   echo '<html>
					<head>
						<script>
							window.alert("Kursus berjaya di padam!");
						</script>
						<meta http-equiv="refresh" content="0; url=../SISCO/tadbir_kursus.php" />
					</head>
				</html>';
		}
		else
		{
			...
			echo '<html>
					<head>
						<script>
							window.alert("Kursus gagal di padam!");
							window.history.go(-1);
						</script>
					</head>
				</html>';
