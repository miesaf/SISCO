<?php
	...
	if (isset($_GET['ID']))
	{
		...
		// delete the entry
		$result = mysql_query("DELETE FROM ANNOUNCEMENTS WHERE ANN_ID = '$ID'") or die (mysql_error()); 
		...
		if ($result)
		{
		   echo '<html>
					<head>
						<script>
							window.alert("Makluman berjaya di padam!");
						</script>
						<meta http-equiv="refresh" content="0; url=../SISCO/senarai_makluman.php" />
					</head>
				</html>';
		}
		else
		{
			...
			echo '<html>
					<head>
						<script>
							window.alert("Makluman gagal di padam!");
							window.history.go(-1);
						</script>
					</head>
				</html>';
