...
<head>
	<?php
		...
		{	
			...
			while($COUNT > 0)
			{
				// Variables from tadbir_course.php
				$IDA		= $_POST['IDA' . $COUNT];
				$ID_PROG	= $_POST['ID_PROG' . $COUNT];
				$PROG		= $_POST['PROG' . $COUNT];
			
				//SQL query command
				$sql="UPDATE COURSE SET CR_ID = '$ID_PROG', CR_NAME = '$PROG' WHERE CR_ID = '$IDA')";
			...
			if ($exe_sql)
			{
				echo '<html>
						<head>
							<script>
								window.alert("Pendaftaran berjaya!\nCourse telah di simpan ke dalam pangkalan data.");
							</script>
							<meta http-equiv="refresh" content="0; url=tadbir_kursus.php" />
						</head>
					</html>';
			}
			else
			{
				...
				echo '<html>
						<head>
							<script>
								window.alert("Pendaftaran gagal!\nCourse tidak di simpan ke dalam pangkalan data");
								window.history.go(-1);
							</script>
						</head>
					</html>';
    ...
    <title>[SISCO] - Kursus (Perkakas Pembangun)</title>
...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-lg-12">
                        <h1 class="page-header">Pinda Kursus (Pukal)</h1>
						<!-- mula     ##########################################################################################     mula -->
						<form id="tambah_course" action ="pinda_kursus.php" method ="POST">
							<input type="hidden" name="DAFTAR" value="YES">
							<?php
								// create the listing query
								$sql = "SELECT * FROM COURSE ORDER BY CR_ID";
							...
							<table border="0" align="center" cellspacing="2" cellpadding="2">
								...
								<td align="center" valign="top">
									<table style="BORDER-COLLAPSE: collapse" bordercolor="#cccccc" align="center" cellspacing="0" cellpadding="0" border="1">
									<tbody>
										<tr>
											<tr>
												<td colspan="5" align="center" height="24" background="../JCIS/images/1_background_table.gif">
													<div align="center">
														<b>Senarai Course</b>
													</div>
												</td>
											...
											<tr>
												<td valign="top">
												<table border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#cccccc" style="BORDER-COLLAPSE: collapse">
												<tbody>
													<td align="center"><b>ID Kursus</b></td>
													<td align="center"><b>Kursus</b></td>
								<?php
								$BIL == 0;
								while ($row = mysql_fetch_array($result))
								{
									...
									<tr>
										<td align=center>
											<input type="hidden" name="IDA<?php print($BIL); ?>" value="<?php print($ID_PROG); ?>">
											<input type="text" maxlength="6" size="4" name="ID_PROG<?php print($BIL); ?>" value="<?php print($ID_PROG); ?>">
										</td>
										<td align=left>
											<input type="text" maxlength="50" size="50" name="PROG<?php print($BIL); ?>" value="<?php print($PROG); ?>">
										</td>
									</tr>
									<?php
								}
								if($BIL == 0)
								{
									...
									<tr>
										<td align='center' colspan='5'>
											<i> Tiada kursus untuk dipaparkan </i>
										</td>
									</tr>
									<?php
								}
								?>
													<tr>
														<td align="center" colspan="2">
															<input type="hidden" name="BIL" value="<?php print($BIL); ?>">
															<button type="submit" form="tambah_course" value="Submit">Pinda</button>
														</td>
													</tr>
												</tbody>
												</table>
	...
	<script language="JavaScript">
		...
		{
			var del = confirm("Adakah anda pasti untuk memadam kursus ini?");
			if (del == true)
			{
				window.location.assign("padam_kursus.php?ID=" + nums);
			} else 
			{
				alert("Kursus tidak di padam.");
			}
		}
	</script>
